...
def _demo_password() -> str:
    """密码只能来自环境变量（登录时校验）：硬编码进仓库等于共享账号向所有读者开放。"""
    password = os.getenv("ECHOGUIDE_DEMO_PASSWORD")
    if not password:
        raise SystemExit("[demo_benchmark] 缺少环境变量 ECHOGUIDE_DEMO_PASSWORD")
    return password
...
def prepare_demo_data(client: httpx.Client) -> None:
    ...
    client.post(
        "/personal/todo",
        json={"content": "提交 Agent 实验报告", "kind": "ddl", "due_at": (today + timedelta(days=5)).isoformat()},
    ).raise_for_status()
...
def aggregate(records: Iterable[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    labels = sorted(
        {
            CASE_BY_ID[row["case_id"]].get("expected_domain")
            for row in rows
            if CASE_BY_ID[row["case_id"]].get("expected_domain")
        }
    )
    f1_values = []
    for label in labels:
        tp = sum(
            row.get("domain") == label and CASE_BY_ID[row["case_id"]].get("expected_domain") == label for row in rows
        )
        fp = sum(
            row.get("domain") == label and CASE_BY_ID[row["case_id"]].get("expected_domain") != label for row in rows
        )
    ...
    gate_tp = sum(
        expected and predicted for expected, predicted in zip(expected_complex, predicted_complex, strict=False)
    )
    ...
    return {
        ...
        "domain_accuracy": round(
            sum((row.get("checks") or {}).get("domain", False) for row in rows) / max(1, len(rows)), 4
        ),
        "domain_macro_f1": round(sum(f1_values) / max(1, len(f1_values)), 4),
        "profile_accuracy": round(
            sum((row.get("checks") or {}).get("profile", False) for row in rows) / max(1, len(rows)), 4
        ),
        "runtime_deep_fallback_rate": round(
            sum(
        ...
        "complexity_accuracy": round(
            sum((row.get("checks") or {}).get("mode", False) for row in rows) / max(1, len(rows)), 4
        ),
        ...
        "tool_success_rate": round(
            sum((row.get("checks") or {}).get("tools", False) for row in rows) / max(1, len(rows)), 4
        ),
        ...
        "llm_classifier_rate": round(
            sum(exe.get("classifier_stage") == "llm" for exe in executions) / max(1, len(executions)), 4
        ),
...
def measure_rag(client: httpx.Client) -> Dict[str, Any]:
    """对版本化 HTTP RAG 探针逐条请求。

    指标聚合复用 evaluation.evaluator.compute_retrieval_metrics（精确标题匹配），
    与 retrieval_* 基准同口径——此前这里用双向子串模糊匹配，同名的
    "HitRate@5" 两套口径不可比。
    """
    payload = json.loads(RAG_CASES_PATH.read_text(encoding="utf-8"))
    cases = payload["cases"]
    titles_per_case: List[List[str]] = []
    expected_per_case: List[List[str]] = []
    for case in cases:
        try:
            ...
            results = []
        titles_per_case.append([str(item.get("title", "")) for item in results])
        expected_per_case.append(case["relevant_titles"])

    metrics = compute_retrieval_metrics(titles_per_case, expected_per_case, top_k=5)
    return {
        "cases": metrics.get("total", len(cases)),
        "hit_rate_at_5": metrics["hit_rate@K"],
        "recall_at_5": metrics["recall@K"],
        "mrr": metrics["mrr"],
        "records": metrics.get("cases", []),
    }
    ...
    return "\n".join(
        [
            ...
            "> 消融：专属工具成功率 {adaptive_specialized}，改用通用 RAG 后为 {generic_specialized}；依赖 DAG 成功率 {adaptive_dag}，强制单 Agent 后为 {single_dag}。".format(
                ...
                generic_specialized=percentage(
                    report["summary"].get("generic_rag", {}).get("specialized_tool_success_rate")
                ),
...
def main() -> int:
    ...
    parser.add_argument(
        "--timeout", type=float, default=120.0, help="单个 HTTP 请求超时秒数；超时会记为失败并继续后续用例。"
    )
